...
var source = (() => {
  ...
  // src/NeloMangaNET/main.ts
  var main_exports = {};
  __export(main_exports, {
    NeloMangaNET: () => NeloMangaNET
  });
  init_buffer();

  // src/generic/Mangabox.ts
  init_buffer();
  ...
  // src/generic/MangaboxInterceptor.ts
  init_buffer();
  var import_types2 = __toESM(require_lib(), 1);
  var MangaboxInterceptor = class extends import_types2.PaperbackInterceptor {
    source;
    promise;
  ...
  var MangaboxParser = class {
    async parseMangaDetails($2, mangaId, source) {
      ...
      const title = $2("img", context).attr("alt")?.trim() ?? "";
      const image = encodeURI(
        await this.getImageSrc($2("img", context), source) ?? ""
      );
      const secondaryTitleBox = $2(".story-alternative", context).first().text().trim();
      const secondaryTitles = secondaryTitleBox.replace(/^Alternative\s*:\s*/i, "").split(";").map((title2) => title2.trim()).filter((title2) => title2.length > 0);
      const authors = $2('li:contains("Author(s)") a').map((_, el) => $2(el).text().trim()).get().join(", ");
      const synopsis = Application.decodeHTMLEntities(
        $2("#contentBox", context).first().text().trim()
      );
      const shareUrl = `${source.domain}/manga/${mangaId}`;
      const ratingParsed = $2("#rate_row_cmd").text().trim();
      const rating = (Number(ratingParsed.match(/rate\s*:\s*([\d.]+)/)?.[1]) || 0) / 10 * 2;
      const parsedStatus = $2("li:contains(Status)", context).text().trim().toUpperCase();
      let status;
      ...
      const genres = [];
      for (const obj of $2("a", $2("li.genres", context)).toArray()) {
        const title2 = $2(obj).text().trim();
        const id = this.idCleaner($2(obj).attr("href") ?? "");
        if (!title2 || !id) continue;
        if (["adult", "mature", "smut"].includes(title2.toLowerCase())) {
    ...
    parseChapterList($2, sourceManga, source) {
      const chapters = [];
      const nodeArray = $2(".row", ".chapter-list").toArray();
    ...
    async parseChapterDetails($2, chapter, source) {
      const pages = [];
      for (const obj of $2("img", "div.container-chapter-reader").toArray()) {
    ...
    async parseDiscoverSections($2, section, source) {
      const items = [];
      for (const obj of $2("div.list-truyen-item-wrap").toArray()) {
        const image = encodeURI(
          await this.getImageSrc($2("img", obj), source) ?? ""
        );
        const title = $2("img", obj).attr("alt")?.trim() ?? "";
        const id = this.idCleaner($2("a", obj).attr("href") ?? "");
        const subtitle = $2("a.list-story-item-wrap-chapter", obj).first().text().trim();
        if (!id || !title) {
          continue;
        }
        switch (section.type) {
          case import_types3.DiscoverSectionType.featured:
            items.push({
              mangaId: id,
              imageUrl: image,
            ...
            break;
          case import_types3.DiscoverSectionType.prominentCarousel:
            items.push({
              mangaId: id,
              imageUrl: image,
              title: Application.decodeHTMLEntities(title),
            ...
            break;
          case import_types3.DiscoverSectionType.simpleCarousel:
            items.push({
              mangaId: id,
              imageUrl: image,
              title: Application.decodeHTMLEntities(title),
    ...
    async parseSearchTags($2) {
      const genres = [];
      const context = $2('h3:contains("GENRES")').parent();
      for (const obj of $2("td", context).toArray()) {
        const title = $2("a", obj).attr("title")?.trim() ?? "";
        const id = this.idCleaner($2("a", obj).attr("href") ?? "");
        if (!id || !title) {
          continue;
        }
    ...
    async parseSearchResults($2, source, query) {
      const results = [];
      if (query.title) {
        for (const obj of $2("div.story_item").toArray()) {
          const image = encodeURI(
            await this.getImageSrc($2("img", obj), source) ?? ""
          );
          const title = $2(".story_name", obj).text().trim();
          const id = this.idCleaner($2("a", obj).attr("href") ?? "");
          const subtitle = $2(".story_chapter", obj).first().text().trim();
          if (!id || !title) {
            continue;
          }
          results.push({
  ...
  var MangaboxGeneric = class {
    ...
    constructor(params) {
      ...
      this.defaultContentRating = params.contentRating ?? import_types4.ContentRating.EVERYONE;
    ...
    async getChapterDetails(chapter) {
      ...
      await this.checkResponseError(response);
      const $2 = load(Application.arrayBufferToUTF8String(buffer));
      return this.parser.parseChapterDetails($2, chapter, this);
    }
    ...
    async getSearchFilters() {
      ...
      return [
        {
          type: "multiselect",
          options: genreTags.tags.sort((a, b) => a.title.localeCompare(b.title)).map((x) => ({
            id: x.id,
            value: x.title
          })),
          id: genreTags.id,
    ...
    async getSearchResults(query, metadata) {
      ...
      await this.checkResponseError(response);
      const $2 = load(Application.arrayBufferToUTF8String(buffer));
      const results = await this.parser.parseSearchResults($2, this, query);
      metadata = !this.parser.isLastPage($2) ? { page: page + 1 } : void 0;
      return {
        items: results,
        metadata
    ...
    constructSearchRequest(page, query) {
      const urlBuilder = new import_types4.URL(this.domain);
  ...
  // src/NeloMangaNET/pbconfig.ts
  init_buffer();
  var import_types5 = __toESM(require_lib(), 1);

  // src/generic/MangaboxHelper.ts
  init_buffer();
  var BASE_VERSION = "1.0.0-alpha.1";
  ...
  // src/NeloMangaNET/main.ts
  var DOMAIN = "https://www.nelomanga.net";
  var NeloMangaNETExtension = class extends MangaboxGeneric {
    constructor() {
      super({
        domain: DOMAIN,
  ...
  var NeloMangaNET = new NeloMangaNETExtension();
