...
var source = (() => {
  ...
  // src/generic/MangaStreamParser.ts
  init_buffer();

  // src/generic/MangaStreamSettingsForm.ts
  init_buffer();
  var import_types3 = __toESM(require_lib(), 1);
  function toBoolean(value) {
    return (value ?? false) === "true";
  }
  function getUsePostIds() {
  ...
  // src/generic/MangaStreamUtils.ts
  init_buffer();
  ...
  var MangaStreamParser = class {
    parseMangaDetails($2, mangaId, source) {
      const titles = [];
      titles.push($2("h1.entry-title").text().trim());
      const altTitles = $2(
      ...
      const description = $2('div[itemprop="description"]  p').text().trim();
      const arrayTags = [];
      for (const tag of $2("a", source.mangaTagSelectorBox).toArray()) {
    ...
    parseChapterList($2, sourceManga, source) {
      ...
      if (sourceManga.mangaId.toUpperCase().endsWith("-RAW") && source.language == "\u{1F1EC}\u{1F1E7}")
        language = "\u{1F1F0}\u{1F1F7}";
      for (const chapter of $2("li", "div#chapterlist").toArray()) {
        const title = $2("span.chapternum", chapter).text().trim().replace(/\s+/g, " ");
    ...
    async parseHomeSection($2, section, source) {
      ...
      for (const manga of mangas.toArray()) {
        ...
        const mangaId = getUsePostIds() ? isNaN(Number(postId)) ? await source.slugToPostId(slug, path) : postId : slug;
        if (!mangaId || !title) {
          console.log(
    ...
    buildFeaturedTitle(mangaId, imageUrl, title) {
      return {
        mangaId,
        imageUrl,
        title,
    ...
    buildPopularTitle(mangaId, imageUrl, title, subtitle) {
      return {
    ...
    buildLatestTitle(mangaId, imageUrl, title, chapterId) {
      return {
  ...
  var MangaStreamGeneric = class {
    ...
    async getSearchResults(query, metadata) {
      ...
      if (query?.title) {
        ...
        ).addQuery("type", getIncludedTagBySection("type", includedTags)).addQuery(
          "order",
    ...
    async getMangaDetails(mangaId) {
      const request = {
        url: getUsePostIds() ? `${this.domain}/?p=${mangaId}/` : `${this.domain}/${this.directoryPath}/${mangaId}/`,
        method: "GET"
      };
      const [response, buffer] = await Application.scheduleRequest(request);
      this.checkResponseError(request, response);
    ...
    async getChapters(sourceManga) {
      const request = {
        url: getUsePostIds() ? `${this.domain}/?p=${sourceManga.mangaId}/` : `${this.domain}/${this.directoryPath}/${sourceManga.mangaId}/`,
        method: "GET"
      };
      const [response, buffer] = await Application.scheduleRequest(request);
      this.checkResponseError(request, response);
    ...
    async getChapterDetails(chap) {
      const request = {
        url: getUsePostIds() ? `${this.domain}/?p=${chap.sourceManga.mangaId}/` : `${this.domain}/${this.directoryPath}/${chap.sourceManga.mangaId}/`,
        method: "GET"
      };
      const [response, buffer] = await Application.scheduleRequest(request);
      this.checkResponseError(request, response);
      ...
      const chapter = chapters.find(
        (x) => $2(x).attr("data-num") === chap.chapterId
      );
      if (!chapter) {
        throw new Error(
    ...
    async getDiscoverSectionItems(section, metadata) {
      ...
      return {
        items: await this.parser.parseHomeSection($2, s, this),
        metadata
      };
    }
    ...
    async convertPostIdToSlug(postId) {
      ...
      const slug = parseSlugArr.slice(-1).pop();
      const path = parseSlugArr.slice(-2).shift();
      return {
        path,
        slug
      };
    ...
    async convertSlugToPostId(slug, path) {
      ...
      if (isNaN(postIdNum)) {
        postIdNum = Number($2("div.bookmark").attr("data-id"));
      }
      if (isNaN(postIdNum)) {
  ...
  // src/LelManga/main.ts
  var DOMAIN_NAME = "https://www.lelmanga.com";
  var LelMangaExtension = class extends MangaStreamGeneric {
    domain = DOMAIN_NAME;
    name = pbconfig_default.name;
    contentRating = pbconfig_default.contentRating;
    language = "\u{1F1EB}\u{1F1F7}";
    mangaSelectorAuthor = "Autheur";
    mangaSelectorArtist = "Artiste";
    configureSections() {
      this.latestUpdatesSection.selectorFunc = ($2) => $2("div.uta", $2("h2:contains(Derni\xE8res Sorties)")?.parent()?.next());
      this.featuredSection.selectorFunc = ($2) => $2(
        "div.bsx",
        $2("h2:contains(Top Managa Aujourd'hui)")?.parent()?.next()
      );
    }
  };
  var LelManga = new LelMangaExtension();
