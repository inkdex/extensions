...
var source = (() => {
  ...
  var parseMangaDetails = async ($2, mangaId) => {
    ...
    const artist = $2('h3:contains("Artist")').next().text().trim() ?? "";
    const arrayTags = [];
    for (const tag of $2(
      "button",
      $2('h3:contains("Genres")').next()
    ).toArray()) {
    ...
    return {
      ...
      mangaInfo: {
        ...
        status,
        author: load(author).text() === "_" ? void 0 : load(author).text(),
        artist: load(artist).text() === "_" ? void 0 : load(artist).text(),
