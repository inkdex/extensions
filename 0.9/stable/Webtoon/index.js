...
var source = (() => {
  ...
  var require_id = __commonJS({
    "node_modules/moment/locale/id.js"(exports, module) {
      ...
      (function(global, factory) {
        ...
        "use strict";
        var id = moment2.defineLocale("id", {
  ...
  var formatDate = (date, language) => {
    switch (language) {
      case "en" /* ENGLISH */:
        return new Date(
          (0, import_moment.default)(date, "MMMM D, YYYY", "en" /* ENGLISH */).toDate()
        );
      case "fr" /* FRENCH */:
        return new Date(
          (0, import_moment.default)(date, "D MMM YYYY", "fr" /* FRENCH */).toDate()
        );
      case "de" /* GERMAN */:
        return new Date(
          (0, import_moment.default)(date, "DD.MM.YYYY", "de" /* GERMAN */).toDate()
        );
      case "es" /* SPANISH */:
        return new Date(
          (0, import_moment.default)(date, "DD-MMM-YYYY", "es" /* SPANISH */).toDate()
        );
      case "th" /* THAI */:
        return new Date((0, import_moment.default)(date, "D MMM YYYY", "th" /* THAI */).toDate());
      case "id" /* INDONESIAN */:
        return new Date(
          (0, import_moment.default)(date, "YYYY MMM D", "id" /* INDONESIAN */).toDate()
        );
      case "zh-hant" /* CHINESE */:
        return new Date((0, import_moment.default)(date, "l", "zh-tw").toDate());
      default:
        return new Date(date);
    }
  };
  var getDateDayFormat = () => {
    return (0, import_moment.default)().locale("en").format("dddd").toUpperCase();
  };
  var haveTrending = (language) => {
    switch (language) {
      case "de" /* GERMAN */:
      case "es" /* SPANISH */:
        return false;
  ...
  // src/Webtoon/WebtoonParser.ts
  init_buffer();
  var import_types3 = __toESM(require_lib(), 1);

  // src/Webtoon/WebtoonSettings.ts
  init_buffer();
  ...
  var WebtoonSettings = class {
    ...
    get languages() {
      return Application.getState(LANGUAGES) ?? [
        "en" /* ENGLISH */
      ];
    }
    set languages(value) {
  ...
  var WebtoonSettingForm = class extends import_types2.Form {
    ...
    getSections() {
      return [
        (0, import_types2.Section)("settings", [
          ...
          (0, import_types2.ToggleRow)(CANVAS_WANTED, {
            title: "Show Canvas",
  ...
  // src/Webtoon/WebtoonParser.ts
  var WebtoonParser = class extends WebtoonSettings {
    BASE_URL;
    MOBILE_URL;
    constructor(BASE_URL2, MOBILE_URL2) {
      super();
      this.BASE_URL = BASE_URL2;
      this.MOBILE_URL = MOBILE_URL2;
    }
    parseDetails($2, mangaId) {
      const detailElement = $2(
    ...
    parseChapter(elem, sourceManga) {
      return {
        ...
        publishDate: formatDate(
          elem.find("a > div.row > div.info > div.sub_info > span.date").text(),
    ...
    parseCanvasGenres($2) {
      return $2("#content ul.challenge li").toArray().filter(
        (elem) => $2(elem).attr("data-genre") && $2(elem).attr("data-genre") !== "ALL"
      ).map((elem) => this.parseCanvasTagFromElement($2(elem)));
    }
  ...
  // src/Webtoon/WebtoonInfra.ts
  var WebtoonInfra = class extends WebtoonParser {
    cheerio = browser_exports;
  ...
  var WebtoonExtention = class extends WebtoonInfra {
    constructor() {
    ...
    getPopularTitles(language) {
      return this.ExecPagedResultsRequest(
        { url: `${this.BASE_URL}/${language}/popular` },
        { page: 0, maxPages: 1 },
        ($2) => this.parsePopularTitles($2)
      );
    }
    getTodayTitles(language, metadata) {
      return this.ExecPagedResultsRequest(
    ...
    getOngoingTitles(language, metadata) {
      return this.ExecPagedResultsRequest(
        { url: `${this.BASE_URL}/${language}/originals` },
        { page: metadata?.page ?? 0, maxPages: 2 },
    ...
    getCompletedTitles(language, metadata) {
      return this.ExecPagedResultsRequest(
        { url: `${this.BASE_URL}/${language}/originals` },
        { page: metadata?.page ?? 0, maxPages: 2 },
    ...
    getCanvasRecommendedTitles(language) {
      return this.ExecRequest(
        { url: `${this.BASE_URL}/${language}/canvas` },
        ($2) => this.parseCanvasRecommendedTitles($2)
      );
    }
    getCanvasPopularTitles(language, metadata, genre) {
      return this.ExecPagedResultsRequest(
        {
        ...
        ($2) => this.parseCanvasPopularTitles($2)
      );
    }
    getTitlesByGenre(language, genre) {
      return this.ExecRequest(
        {
          url: `${this.BASE_URL}/${language}/genres/${genre}`,
          params: { sortOrder: "READ_COUNT" }
        },
        ($2) => this.parseTagResults($2)
      );
    }
    getTitlesByKeyword(language, keyword, metadata) {
      return this.ExecPagedResultsRequest(
        {
    ...
    async getSearchGenres() {
      return [
        ...
        ...await this.ExecRequest(
          ...
          ($2) => this.parseGenres($2)
        ),
        ...this.canvasWanted ? await this.ExecRequest(
          { url: `${this.BASE_URL}/en/canvas` },
          ($2) => this.parseCanvasGenres($2)
        ) : []
      ];
    }
  };
  var Webtoon = new WebtoonExtention();
