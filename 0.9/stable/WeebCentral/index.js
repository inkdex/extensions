...
var source = (() => {
  ...
  var pbconfig_default = {
    ...
    capabilities: [
      ...
      import_types2.SourceIntents.DISCOVER_SECIONS,
      import_types2.SourceIntents.MANGA_SEARCH
    ],
    badges: [],
    developers: [
      {
  ...
  // src/WeebCentral/WeebCentralConfig.ts
  init_buffer();
  var WC_DOMAIN = "https://weebcentral.com/";

  // src/WeebCentral/WeebCentralEnums.ts
  init_buffer();
  ...
  function getShareUrl(mangaId) {
    return `${WC_DOMAIN}/series/${mangaId}`;
  }
  function getRating(rating) {
    return rating === "Yes" ? import_types3.ContentRating.ADULT : import_types3.ContentRating.EVERYONE;
  }

  // src/WeebCentral/WeebCentralInterceptor.ts
  init_buffer();
  ...
  var parseMangaDetails = async ($2, mangaId) => {
    ...
    const description = decodeHTML($2(".whitespace-pre-wrap").text().trim());
    const authors = [];
    for (const authorObj of $2('strong:contains("Author")').siblings("span").toArray()) {
    ...
    const isAdultContent = $2('strong:contains("Adult Content")').next().text().trim();
    const tagSections = [
      {
        id: "included_tag" /* Genres */,
        title: "Genres" /* Genres */,
        tags: genres
      }
    ];
  ...
  var parseChapters = ($2, sourceManga) => {
    ...
    for (const chapterObj of arrChapters) {
      ...
      const title = $2("span.grow.flex.gap-2 span", chapterObj).first().text().trim();
      let chapNum = 0;
      let chapType = "";
      const matches = title.match(floatRegex);
      if (matches && matches[matches.length - 1]) {
        chapNum = parseFloat(matches[matches.length - 1] ?? "0");
        chapType = title.slice(0, -matches[matches.length - 1].length).trim();
      }
      sortingIndex--;
    ...
    return chapters.map((chapter) => {
      if (chapter.title == void 0) {
        throw new Error(`Chapter title is undefined`);
      }
      const matches = chapter.title.match(floatRegex);
      if (matches) {
        const chapType = chapter.title.slice(0, -matches[matches.length - 1].length).trim();
  ...
  var parseRecommendedSection = async ($2) => {
    const recommendedSectionArray = [];
    for (const recommendationObj of $2(
      ...
      const imageUrl = $2("source", recommendationObj).first().attr("srcset") ?? $2("img", recommendationObj).attr("src") ?? "";
  ...
  var parseRecentSection = async ($2) => {
    const recentSectionArray = [];
    for (const recentObj of $2(
      ...
      const chapterId = $2("a.min-w-0", recentObj).attr("href")?.replace(/\/$/, "")?.split("/").slice(-2)[0] ?? "";
      const title = $2("div.font-semibold", recentObj).text().trim() ?? "";
      const imageUrl = $2("a img", recentObj).attr("src") ?? $2("a img", recentObj).attr("data-src") ?? "";
  ...
  var parseHotSection = async ($2) => {
    const hotSectionArray = [];
    for (const hotObj of $2(
      ...
      const imageUrl = $2("source", hotObj).first().attr("srcset") ?? "";
  ...
  var parseIntoTagSection = ($2, id, title, selector) => {
    const tagSection = {
      id,
      title,
      tags: []
    };
    const elements = $2(selector).next().children().toArray();
    for (const element of elements) {
      const title2 = $2("span", element).first().text().trim();
      let id2 = $2("input", element).last().attr("value") ?? "";
      id2 = formatTagId(id2);
      tagSection.tags.push({ id: id2, title: title2 });
    }
    return tagSection;
  };
  var isLastPage = ($2) => {
  ...
  var WeebCentralExtension = class {
    ...
    async getDiscoverSectionItems(section, metadata) {
      let items = [];
      const urlBuilder = new URLBuilder2(WC_DOMAIN);
      ...
      switch (section.id) {
        case "recommended": {
          ...
          items = await parseRecommendedSection($2);
          break;
        }
        case "recent": {
          const [_, buffer] = await Application.scheduleRequest({
          ...
          items = await parseRecentSection($2);
    ...
    async getMangaDetails(mangaId) {
      const request = {
        url: new URLBuilder2(WC_DOMAIN).addPath("series").addPath(mangaId).build(),
        method: "GET"
      };
    ...
    async getChapters(sourceManga) {
      const request = {
        url: new URLBuilder2(WC_DOMAIN).addPath("series").addPath(sourceManga.mangaId).addPath("full-chapter-list").build(),
        method: "GET"
      };
      const [_, buffer] = await Application.scheduleRequest(request);
      const $2 = load(Application.arrayBufferToUTF8String(buffer));
    ...
    async getChapterDetails(chapter) {
      const url = new URLBuilder2(WC_DOMAIN).addPath("chapters").addPath(chapter.chapterId).addPath("images").addQuery("reading_style", "long_strip").build();
    ...
    async getGenres() {
      ...
      const genreTag = tags.find(
        (tag) => tag.id === "included_tag" /* Genres */
      );
      if (genreTag === void 0) {
        throw new Error("Genres tag section not found");
      }
      if (isInvalidTags(genreTag.tags)) {
        throw new Error(`Please reset ${pbconfig_default.name} state in settings`);
      }
    ...
    async getSearchTags() {
      ...
      try {
        const request = {
          url: new URLBuilder2(WC_DOMAIN).addPath("search").build(),
          method: "GET"
        };
        const [_, buffer] = await Application.scheduleRequest(request);
        const $2 = load(Application.arrayBufferToUTF8String(buffer));
        tags = await parseTags($2);
    ...
    async getSearchFilters() {
      ...
      const filters2 = [];
      filters2.push(this.getGenresFilter(tags));
      filters2.push(this.getSeriesStatusFilter(tags));
      filters2.push(this.getSeriesTypeFilter(tags));
      filters2.push(this.getOrderFilter(tags));
      return filters2;
    }
    async getSearchResults(query, metadata) {
      const LIMIT = 32;
      const offset = metadata?.offset ?? 0;
      let newUrlBuilder = new URLBuilder2(WC_DOMAIN).addPath("search").addPath("data").addQuery("sort", "Best Match").addQuery("display_mode", "Full Display").addQuery("limit", LIMIT.toString()).addQuery("offset", offset.toString());
      if (query.title) {
        newUrlBuilder = newUrlBuilder.addQuery("text", query.title);
      }
      newUrlBuilder = newUrlBuilder.addQuery(
        "included_tag" /* Genres */,
        getFilterTagsBySection("included_tag" /* Genres */, query.filters)
      ).addQuery(
        "included_status" /* SeriesStatus */,
        ...
        "included_type" /* SeriesType */,
        getFilterTagsBySection("included_type" /* SeriesType */, query.filters)
      ).addQuery(
        "order" /* Order */,
        getFilterTagsBySection("order" /* Order */, query.filters)
      );
      ...
      metadata = isLastPage($2) ? void 0 : { ...metadata, offset: offset + LIMIT };
      return { items, metadata };
    }
    getGenresFilter(tags) {
      const tag = getTagFromTagStore("included_tag" /* Genres */, tags);
      return {
        id: tag.id,
        title: tag.title,
        type: "multiselect",
    ...
    getSeriesStatusFilter(tags) {
      const tag = getTagFromTagStore("included_status" /* SeriesStatus */, tags);
      return {
        id: tag.id,
        title: tag.title,
        type: "multiselect",
    ...
    getSeriesTypeFilter(tags) {
      const tag = getTagFromTagStore("included_type" /* SeriesType */, tags);
      return {
        id: tag.id,
        title: tag.title,
        type: "multiselect",
    ...
    getOrderFilter(tags) {
      const tag = getTagFromTagStore("order" /* Order */, tags);
      return {
        id: tag.id,
        title: tag.title,
        type: "dropdown",
