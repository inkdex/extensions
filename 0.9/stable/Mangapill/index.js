...
var source = (() => {
  ...
  // src/Mangapill/main.ts
  var main_exports = {};
  __export(main_exports, {
    Mangapill: () => Mangapill,
    MangapillExtension: () => MangapillExtension
  });
  init_buffer();
  ...
  // src/Mangapill/MangapillConfig.ts
  init_buffer();
  var MANGA_PILL_DOMAIN = "https://mangapill.com/";

  // src/Mangapill/MangapillHelper.ts
  init_buffer();
  ...
  // src/Mangapill/MangapillInterceptor.ts
  init_buffer();
  var import_types2 = __toESM(require_lib(), 1);
  var MangapillInterceptor = class extends import_types2.PaperbackInterceptor {
    async interceptRequest(request) {
      request.headers = {
        ...request.headers,
  ...
  // src/Mangapill/MangapillParser.ts
  init_buffer();

  // src/Mangapill/pbconfig.ts
  init_buffer();
  ...
  var parseMangaDetails = async ($2, mangaId) => {
    const title = decodeHTML($2("h1").first().text().trim());
    const image = $2(".lazy").attr("data-src") ?? "";
    const description = decodeHTML($2(".text-sm.text--secondary").text().trim());
    const parsedStatus = $2('label:contains("Status")').siblings().first().text().trim();
    let status;
    switch (parsedStatus) {
      case "publishing":
        status = "Ongoing";
    ...
    const genres = [];
    for (const genreObj of $2('label:contains("Genres")').siblings().toArray()) {
  ...
  var parseChapters = ($2, sourceManga) => {
    const chapters = [];
    const arrChapters = $2("#chapters a").toArray();
    let sortingIndex = 0;
    let hasVolume = false;
    for (const chapterObj of arrChapters) {
      const chapterId = $2(chapterObj).attr("href")?.split("/")[2] ?? "";
      const title = $2(chapterObj).text().trim();
      let chapNum = 0;
      let match = $2(chapterObj).text().trim().match(/Chapter (\d+)/);
      if (match && match[1]) {
        chapNum = parseFloat(match[1]);
      }
      let volume = 1;
      match = $2(chapterObj).text().trim().match(/Group (\d+)/);
      if (match && match[1]) {
        hasVolume = true;
  ...
  var parseChapterDetails = async ($2, mangaId, chapterId) => {
    const pages = [];
    for (const pageObj of $2("picture > img").get()) {
      const page = $2(pageObj).attr("data-src") ?? $2(pageObj).attr("src") ?? "";
  ...
  var parseRecentSection = async ($2) => {
    ...
    for (const recentObj of $2("> div", $2(".grid-cols-2").first()).toArray()) {
      const id = $2("a.text-secondary", recentObj).attr("href")?.split("/")[2] ?? "";
      const title = $2("div a > div", recentObj).first().text().trim() ?? "";
      const imageUrl = $2("a img", recentObj).attr("src") ?? $2("a img", recentObj).attr("data-src") ?? "";
      const subtitle = $2(".text-secondary", recentObj).text().trim() ?? "";
      const chapterId = $2("a.relative.block").attr("href")?.split("/")[2] ?? "";
  ...
  var parseTrendingSection = async ($2) => {
    const hotSectionArray = [];
    for (const trendingObj of $2("> div", $2(".grid-cols-2").last()).toArray()) {
      const id = $2("a.relative", trendingObj).attr("href")?.split("/")[2] ?? "";
      const title = $2("div a > div", trendingObj).first().text().trim() ?? "";
      const imageUrl = $2("a img", trendingObj).attr("src") ?? $2("a img", trendingObj).attr("data-src") ?? "";
      const subtitle = $2(".font-black", trendingObj).text().trim() ?? "";
  ...
  var parseSearch = async ($2) => {
    const itemArray = [];
    for (const item of $2(".my-3.grid > div").toArray()) {
      const id = $2("a", item).attr("href")?.split("/")[2] ?? "";
      if (id == "" || typeof id != "string") throw new Error("Id is empty");
      const title = $2("div a", item).text().trim() ?? "";
      const image = $2("a img", item).attr("src") ?? $2("a img", item).attr("data-src") ?? "";
      const subtitle = $2(".text-secondary", item).text().trim() ?? "";
      itemArray.push({
        imageUrl: image,
        title: decodeHTML(title),
        mangaId: id,
  ...
  var parseTags = async ($2) => {
    const genres = [];
    for (const genreObj of $2(".grid.gap-1 div").toArray()) {
      const title = $2(genreObj).text().trim();
      let id = $2("input", genreObj).attr("value") ?? title;
      id = formatTagId(id);
  ...
  var MangapillExtension = class {
    ...
    async getDiscoverSections() {
      return [
        ...
        { id: "genre", title: "Genres", type: import_types4.DiscoverSectionType.genres }
      ];
    }
    async getDiscoverSectionItems(section, metadata) {
      let items = [];
      const urlBuilder = new URLBuilder2(MANGA_PILL_DOMAIN);
      switch (section.id) {
        case "trending": {
          ...
          items = await parseTrendingSection($2);
          break;
        }
        case "recent": {
          const [_, buffer] = await Application.scheduleRequest({
    ...
    async getMangaDetails(mangaId) {
      const request = {
        url: new URLBuilder2(MANGA_PILL_DOMAIN).addPath("manga").addPath(mangaId).build(),
        method: "GET"
      };
    ...
    async getChapters(sourceManga) {
      const request = {
        url: new URLBuilder2(MANGA_PILL_DOMAIN).addPath("manga").addPath(sourceManga.mangaId).build(),
        method: "GET"
      };
      const [_, buffer] = await Application.scheduleRequest(request);
      const $2 = load(Application.arrayBufferToUTF8String(buffer));
    ...
    async getChapterDetails(chapter) {
      const url = new URLBuilder2(MANGA_PILL_DOMAIN).addPath("chapters").addPath(chapter.chapterId).build();
    ...
    async getSearchTags() {
      try {
        const request = {
          url: new URLBuilder2(MANGA_PILL_DOMAIN).addPath("search").build(),
          method: "GET"
        };
        const [_, buffer] = await Application.scheduleRequest(request);
        const $2 = load(Application.arrayBufferToUTF8String(buffer));
        return await parseTags($2);
      } catch (error) {
    ...
    async getSearchResults(query, metadata) {
      let newUrlBuilder = new URLBuilder2(
        MANGA_PILL_DOMAIN
      ).addPath("search");
      if (query.title) {
  ...
  var Mangapill = new MangapillExtension();
